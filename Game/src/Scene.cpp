...
void Scene::Setup()
{
    ...
    for (int x = 0; x < 1; x++)
    {
        for (int z = 0; z < 1; z++)
        {
            Entity meshEntity = gCoordinator.CreateEntity();
            auto modelTransform = Transform(Vec3(x * 2, 0, 5 + z * 2), Quat(Vec3(1, 0, 0), 0.0));
            gCoordinator.AddComponent<Transform>(meshEntity, modelTransform);
            gCoordinator.AddComponent<Mesh>(meshEntity, Mesh(Furina));
        }
    }
    ...
    rendererM = std::make_shared<RendererAVX>(
        RendererAVX(APP_VIRTUAL_WIDTH, APP_VIRTUAL_HEIGHT)
    );
}
