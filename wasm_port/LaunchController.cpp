...
void LaunchController::Update()
{
     ...
     if (m_GameState->State == LaunchMode && App::IsKeyPressed(' '))
     {
...
void LaunchController::CalculateLaunchPower()
{
    ...
    Vec2 direction = m_ScreenPoint - mousePoint;
    ...
    m_LaunchDirection = (m_ScreenPoint - mousePoint).Normalize();
    m_LaunchAngle = Vec2::GetAngleBetween(right2D, m_LaunchDirection);
    m_LaunchRenderPoint = m_ScreenPoint + m_LaunchDirection * m_Power * maxDirectionMag * -1.0;
}
